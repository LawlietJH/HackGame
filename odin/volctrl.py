...
class VolumeCtrl:
	...
	def __init__(self):
		...
		self.vol_origin = self.getVol()
		self.vol = self.getVol()
	
	def getVol(self):
		vol = self.ev.GetMasterVolumeLevelScalar()
		self.vol = round(vol*100)
		return self.vol
		
	def setVol(self, vol=50):
		try:
			if vol < 0.01: vol = 0
			elif vol >= 0 and vol <= 1:
				self.vol = round(vol*100)
			elif vol > 100:
				return False
			else:
				self.vol = round(vol)
				vol = vol/100
			self.ev.SetMasterVolumeLevelScalar(vol)
		except:
			return False
		else:
			return True
	...
	def getMute(self):
		mute = self.ev.GetMute()
		if mute == 0: return False
		else: return True
	
	def setMute(self, v=False): self.ev.SetMute(v)
	
	def getChannelCount(self): return self.ev.GetChannelCount()
	
	def getChannelVol(self, c=0): return self.ev.GetChannelVolumeLevelScalar(c)
